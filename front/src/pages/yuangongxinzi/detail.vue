	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yuangongxinzi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">员工账号</div>
						<div class="text "  >{{detail.yuangongzhanghao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">工资月份</div>
						<div class="text "  >{{detail.gongziyuefen}}</div>
					</div>
					<div class="item">
						<div class="lable">基本工资</div>
						<div class="text "  >{{detail.jibengongzi}}</div>
					</div>
					<div class="item">
						<div class="lable">绩效工资</div>
						<div class="text "  >{{detail.jixiaogongzi}}</div>
					</div>
					<div class="item">
						<div class="lable">加班工资</div>
						<div class="text "  >{{detail.jiabangongzi}}</div>
					</div>
					<div class="item">
						<div class="lable">迟到早退</div>
						<div class="text "  >{{detail.chidaozaotui}}</div>
					</div>
					<div class="item">
						<div class="lable">请假</div>
						<div class="text "  >{{detail.qingjia}}</div>
					</div>
					<div class="item">
						<div class="lable">个税</div>
						<div class="text "  >{{detail.geshui}}</div>
					</div>
					<div class="item">
						<div class="lable">全勤奖</div>
						<div class="text "  >{{detail.quanqinjiang}}</div>
					</div>
					<div class="item">
						<div class="lable">实发工资</div>
						<div class="text "  >{{detail.shifagongzi}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.beizhu}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yuangongxinzi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yuangongxinzi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yuangongxinzi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yuangongxinziAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此员工薪资？') .then(_ => {
					this.$http.post('yuangongxinzi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
