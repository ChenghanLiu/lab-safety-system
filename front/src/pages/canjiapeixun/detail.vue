	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/canjiapeixun?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.peixunleixing}}</div>
					</div>
					<div class="item">
						<div class="lable">参加时间</div>
						<div class="text "  >{{detail.canjiashijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('canjiapeixun','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('canjiapeixun','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="参加详情" name="first">
					<div class="ql-snow ql-editor" v-html="detail.canjiaxiangqing"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.peixuntupian) {
							this.detailBanner = this.detail.peixuntupian.split(",w").length>1?[this.detail.peixuntupian]:this.detail.peixuntupian.split(',');
						}
						this.$forceUpdate();
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/canjiapeixun', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/canjiapeixunAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此参加培训？') .then(_ => {
					this.$http.post('canjiapeixun/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
