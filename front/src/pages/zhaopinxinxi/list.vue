...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('zhaopinleixing/list',{params: {sort: 'id',order: 'asc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'zhaopinleixing'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.curFenlei != '全部') searchWhere.zhaopinleixing = this.curFenlei;
