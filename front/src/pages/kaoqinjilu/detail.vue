	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/kaoqinjilu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">月份</div>
						<div class="text "  >{{detail.yuefen}}</div>
					</div>
					<div class="item">
						<div class="lable">出勤天数</div>
						<div class="text "  >{{detail.chuqintianshu}}</div>
					</div>
					<div class="item">
						<div class="lable">缺勤天数</div>
						<div class="text "  >{{detail.queqintianshu}}</div>
					</div>
					<div class="item">
						<div class="lable">迟到天数</div>
						<div class="text "  >{{detail.chidaotianshu}}</div>
					</div>
					<div class="item">
						<div class="lable">早退天数</div>
						<div class="text "  >{{detail.zaotuitianshu}}</div>
					</div>
					<div class="item">
						<div class="lable">请假天数</div>
						<div class="text "  >{{detail.qingjiatianshu}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('kaoqinjilu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('kaoqinjilu','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/kaoqinjilu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/kaoqinjiluAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此考勤记录？') .then(_ => {
					this.$http.post('kaoqinjilu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
