	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gongsixinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">组织架构</div>
						<div class="text "  >{{detail.zuzhijiagou}}</div>
					</div>
					<div class="item">
						<div class="lable">资质荣誉</div>
						<div class="text "  >{{detail.zizhirongyu}}</div>
					</div>
					<div class="item">
						<div class="lable">公司电话</div>
						<div class="text "  >{{detail.gongsidianhua}}</div>
					</div>
					<div class="item">
						<div class="lable">公司邮箱</div>
						<div class="text "  >{{detail.gongsiyouxiang}}</div>
					</div>
					<div class="item">
						<div class="lable">公司地址</div>
						<div class="text "  >{{detail.gongsidizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">创立时间</div>
						<div class="text "  >{{detail.chuanglishijian}}</div>
					</div>
					<div class="item">
						<div class="lable">经营范围</div>
						<div class="text "  >{{detail.jingyingfanwei}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('gongsixinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('gongsixinxi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="公司简介" name="first">
					<div class="ql-snow ql-editor" v-html="detail.gongsijianjie"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.gongsifengmian) {
							this.detailBanner = this.detail.gongsifengmian.split(",w").length>1?[this.detail.gongsifengmian]:this.detail.gongsifengmian.split(',');
						}
						this.$forceUpdate();
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/gongsixinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/gongsixinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此公司信息？') .then(_ => {
					this.$http.post('gongsixinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
