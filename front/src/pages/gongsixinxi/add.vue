<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="公司封面" v-if="type!='cross' || (type=='cross' && !ro.gongsifengmian)" prop="gongsifengmian">
				<file-upload
					tip="点击上传公司封面"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="公司封面" prop="gongsifengmian">
				<img v-if="ruleForm.gongsifengmian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.gongsifengmian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.gongsifengmian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="组织架构" prop="zuzhijiagou">
				<el-input v-model="ruleForm.zuzhijiagou" 
					placeholder="组织架构" clearable :disabled=" false  ||ro.zuzhijiagou"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="资质荣誉" prop="zizhirongyu">
				<el-input v-model="ruleForm.zizhirongyu" 
					placeholder="资质荣誉" clearable :disabled=" false  ||ro.zizhirongyu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="公司电话" prop="gongsidianhua">
				<el-input v-model="ruleForm.gongsidianhua" 
					placeholder="公司电话" clearable :disabled=" false  ||ro.gongsidianhua"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="公司邮箱" prop="gongsiyouxiang">
				<el-input v-model="ruleForm.gongsiyouxiang" 
					placeholder="公司邮箱" clearable :disabled=" false  ||ro.gongsiyouxiang"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="创立时间" prop="chuanglishijian">
				<el-date-picker
			...
			<el-form-item class="add-item" label="经营范围" prop="jingyingfanwei">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="经营范围"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`gongsixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`gongsixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
