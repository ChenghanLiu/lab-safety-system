<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="部门" prop="bumen">
				<el-select v-model="ruleForm.bumen" placeholder="请选择部门" :disabled=" false  ||ro.bumen" >
					<el-option
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/bumen/bumen', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.bumenOptions = res.data.data;
					}
				});
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yuangong/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
