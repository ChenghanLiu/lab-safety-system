<template>
	<div class="center-preview">
		...
		<div class="center-info">
			...
			<div class="img-box" v-if="userTableName=='yuangong'">
				<img :src="sessionForm.touxiang?baseUrl + sessionForm.touxiang:require('@/assets/avator.png')">
			</div>
			<div class="img-box" v-if="userTableName=='renshizhuguan'">
				<img :src="sessionForm.touxiang?baseUrl + sessionForm.touxiang:require('@/assets/avator.png')">
			</div>
			<div class="info-item1" v-if="userTableName=='yuangong'">
				<span class="icon iconfont "></span>
				<div class="label">员工账号：</div>
				<div class="text">{{sessionForm.yuangongzhanghao}}</div>
			</div>
			<div class="info-item2" v-if="userTableName=='yuangong'">
				<span class="icon iconfont "></span>
				<div class="label">员工姓名：</div>
				<div class="text">{{sessionForm.yuangongxingming}}</div>
			</div>
			<div class="info-item3" v-if="userTableName=='yuangong'">
				<span class="icon iconfont "></span>
				<div class="label">性别：</div>
				<div class="text">{{sessionForm.xingbie}}</div>
			</div>
			<div class="info-item4" v-if="userTableName=='yuangong'">
				<span class="icon iconfont "></span>
				<div class="label">联系电话：</div>
				<div class="text">{{sessionForm.lianxidianhua}}</div>
			</div>
			<div class="info-item5" v-if="userTableName=='yuangong'">
				<span class="icon iconfont "></span>
				<div class="label">部门：</div>
				<div class="text">{{sessionForm.bumen}}</div>
			</div>
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			<el-tab-pane label="个人中心">
				<el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
					<el-form-item class="center-item" v-if="userTableName=='yuangong'" label="员工账号" prop="yuangongzhanghao">
						<el-input v-model="sessionForm.yuangongzhanghao" placeholder="员工账号" readonly></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='yuangong'" label="员工姓名" prop="yuangongxingming">
						<el-input v-model="sessionForm.yuangongxingming" placeholder="员工姓名" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='yuangong'" label="性别" prop="xingbie">
						<el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
							<el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='yuangong'" label="联系电话" prop="lianxidianhua">
						<el-input v-model="sessionForm.lianxidianhua" placeholder="联系电话" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='yuangong'" label="头像" prop="touxiang">
						<file-upload
							tip="点击上传头像"
							action="file/upload"
							:limit="1"
					...
					<el-form-item class="center-item" v-if="userTableName=='yuangong'" label="部门" prop="bumen">
						<el-select v-model="sessionForm.bumen" placeholder="请选择部门" >
							<el-option v-for="(item, index) in dynamicProp.bumen" :key="index" :label="item" :value="item"></el-option>
						</el-select>
					</el-form-item>
	...
	export default {
		...
		methods: {
			init() {
				...
				if ('yuangong' == this.userTableName) {
					this.$http.get('option/bumen/bumen', {emulateJSON: true}).then(res => {
						if (res.data.code == 0) {
						  this.dynamicProp.bumen = res.data.data;
						  this.$forceUpdate()
						}
					});
				}
			...
			async updatePassword(){
				this.$refs["passwordForm"].validate(async valid => {
					if (valid) {
						...
						if (this.userTableName == 'yuangong') {
						}
