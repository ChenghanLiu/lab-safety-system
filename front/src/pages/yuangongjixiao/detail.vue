	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yuangongjixiao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">绩效月份</div>
						<div class="text "  >{{detail.jixiaoyuefen}}</div>
					</div>
					<div class="item">
						<div class="lable">员工考勤</div>
						<div class="text "  >{{detail.yuangongkaoqin}}</div>
					</div>
					<div class="item">
						<div class="lable">工作态度</div>
						<div class="text "  >{{detail.gongzuotaidu}}</div>
					</div>
					<div class="item">
						<div class="lable">业务技能</div>
						<div class="text "  >{{detail.yewujineng}}</div>
					</div>
					<div class="item">
						<div class="lable">工作绩效</div>
						<div class="text "  >{{detail.gongzuojixiao}}</div>
					</div>
					<div class="item">
						<div class="lable">奖惩加减分</div>
						<div class="text "  >{{detail.jiangchengjiajianfen}}</div>
					</div>
					<div class="item">
						<div class="lable">总得分</div>
						<div class="text "  >{{detail.zongdefen}}</div>
					</div>
					<div class="item">
						<div class="lable">评价等级</div>
						<div class="text "  >{{detail.pingjiadengji}}</div>
					</div>
					<div class="item">
						<div class="lable">点评</div>
						<div class="text "  >{{detail.dianping}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yuangongjixiao','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yuangongjixiao','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yuangongjixiao', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yuangongjixiaoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此员工绩效？') .then(_ => {
					this.$http.post('yuangongjixiao/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
