<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="培训名称" prop="peixunmingcheng" >
					<el-input v-model="ruleForm.peixunmingcheng" placeholder="培训名称" clearable  :readonly="ro.peixunmingcheng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="培训名称" prop="peixunmingcheng" >
					<el-input v-model="ruleForm.peixunmingcheng" placeholder="培训名称" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="培训类型" prop="peixunleixing" >
					<el-input v-model="ruleForm.peixunleixing" placeholder="培训类型" clearable  :readonly="ro.peixunleixing"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="培训类型" prop="peixunleixing" >
					<el-input v-model="ruleForm.peixunleixing" placeholder="培训类型" readonly></el-input>
				</el-form-item>
				<el-form-item class="upload" v-if="type!='info' && !ro.peixuntupian" label="培训图片" prop="peixuntupian" >
					<file-upload
						tip="点击上传培训图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item class="upload" v-else-if="ruleForm.peixuntupian" label="培训图片" prop="peixuntupian" >
					<img v-if="ruleForm.peixuntupian.substring(0,4)=='http'&&ruleForm.peixuntupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.peixuntupian" width="100" height="100">
					<img v-else-if="ruleForm.peixuntupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.peixuntupian.split(',')[0]" width="100" height="100">
				...
				<el-form-item class="date" v-if="type!='info'" label="参加时间" prop="canjiashijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.canjiashijian" label="参加时间" prop="canjiashijian" >
			...
			<el-form-item v-if="type!='info'"  label="参加详情" prop="canjiaxiangqing" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.canjiaxiangqing" 
					class="editor" 
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.canjiaxiangqing" label="参加详情" prop="canjiaxiangqing" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.canjiaxiangqing"></span>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.canjiashijian = this.getCurDate()
		},
		methods: {
			// 下载
			download(file){
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='peixunleixing'){
							this.ruleForm.peixunleixing = obj[o];
							this.ro.peixunleixing = true;
							continue;
						}
						if(o=='peixuntupian'){
						...
						if(o=='canjiaxiangqing'){
							this.ruleForm.canjiaxiangqing = obj[o];
							this.ro.canjiaxiangqing = true;
							continue;
						}
						if(o=='zhanghao'){
				...
				this.$http({
					...
					if (data && data.code === 0) {
						var json = data.data;
						if(((json.yuangongzhanghao!=''&&json.yuangongzhanghao) || json.yuangongzhanghao==0) && this.$storage.get("role")!="管理员"){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.canjiaxiangqing = this.ruleForm.canjiaxiangqing.replace(reg,'../../../django0bbzy4mb/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
