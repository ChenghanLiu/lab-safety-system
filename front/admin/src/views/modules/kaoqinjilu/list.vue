		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="月份" prop="yuefen">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">月份</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('kaoqinjilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kaoqinjilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('kaoqinjilu','考勤统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						考勤统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","width":"100%","padding":"0 20px 20px","fontSize":"14px","color":"#000","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kaoqinjilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('kaoqinjilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('kaoqinjilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `kaoqinjilu/valueMul/yuangongxingming?yColumnNameMul=chuqintianshu,queqintianshu,chidaotianshu,zaotuitianshu,qingjiatianshu`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							legendObj.data = [
								'出勤天数'.split('/').length>1?'出勤天数'.split('/')[0]:'出勤天数',
								'缺勤天数'.split('/').length>1?'缺勤天数'.split('/')[0]:'缺勤天数',
								'迟到天数'.split('/').length>1?'迟到天数'.split('/')[0]:'迟到天数',
								'早退天数'.split('/').length>1?'早退天数'.split('/')[0]:'早退天数',
								'请假天数'.split('/').length>1?'请假天数'.split('/')[0]:'请假天数',
							]
							...
							let seriesObj = [
								{
									data: yAxis1,
									type: '出勤天数'.split('/').length>1?'出勤天数'.split('/')[1]:'line',
									name: '出勤天数'.split('/').length>1?'出勤天数'.split('/')[0]:'出勤天数',
								},
								{
									data: yAxis2,
									type: '缺勤天数'.split('/').length>1?'缺勤天数'.split('/')[1]:'line',
									name: '缺勤天数'.split('/').length>1?'缺勤天数'.split('/')[0]:'缺勤天数',
								},
								{
									data: yAxis3,
									type: '迟到天数'.split('/').length>1?'迟到天数'.split('/')[1]:'line',
									name: '迟到天数'.split('/').length>1?'迟到天数'.split('/')[0]:'迟到天数',
								},
								{
									data: yAxis4,
									type: '早退天数'.split('/').length>1?'早退天数'.split('/')[1]:'line',
									name: '早退天数'.split('/').length>1?'早退天数'.split('/')[0]:'早退天数',
								},
								{
									data: yAxis5,
									type: '请假天数'.split('/').length>1?'请假天数'.split('/')[1]:'line',
									name: '请假天数'.split('/').length>1?'请假天数'.split('/')[0]:'请假天数',
								},
							]
			...
			getDataList() {
				...
				if(user.yuangongxingming){
					params['yuangongxingming'] = user.yuangongxingming
				}
				this.$http({
					url: "kaoqinjilu/page",
					method: "get",
