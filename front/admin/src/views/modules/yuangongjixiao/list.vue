<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuangongjixiao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yuangongjixiao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuangongjixiao','绩效评分统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						绩效评分统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuangongjixiao','评价等级统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						评价等级统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","width":"100%","padding":"0 20px 20px","fontSize":"14px","color":"#000","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiangchengjiajianfen"
						label="奖惩加减分">
						<template slot-scope="scope">
							{{scope.row.jiangchengjiajianfen}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="zongdefen"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuangongjixiao','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuangongjixiao','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yuangongjixiao','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<el-dialog
			...
			width="800">
			<div id="zongdefenChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
			...
			width="800">
			<div id="pingjiadengjiChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var zongdefenChart1 = echarts.init(document.getElementById("zongdefenChart1"),'macarons');
					this.$http({
						url: `yuangongjixiao/value/yuangongxingming/zongdefen`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].yuangongxingming);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].yuangongxingming
								})
							}
							var option = {};
							let titleObj = this.bar.title
							titleObj.text = '绩效评分统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							zongdefenChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								zongdefenChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var pingjiadengjiChart2 = echarts.init(document.getElementById("pingjiadengjiChart2"),'macarons');
					this.$http({
						url: "yuangongjixiao/typeStat/pingjiadengji/xingming",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								yAxis1.push(parseFloat((res[i].优秀)));
								yAxis2.push(parseFloat((res[i].良好)));
								yAxis3.push(parseFloat((res[i].一般)));
								yAxis4.push(parseFloat((res[i].差)));
								pArray.push({
									name: res[i].pingjiadengji
								})
							}
							var option = {};
							let titleObj = this.bar.title
							titleObj.text = '评价等级统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							pingjiadengjiChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								pingjiadengjiChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.jixiaoyuefen!='' && this.searchForm.jixiaoyuefen!=undefined){
					params['jixiaoyuefen'] = this.searchForm.jixiaoyuefen
				}
				if(this.searchForm.yuangongkaoqin!='' && this.searchForm.yuangongkaoqin!=undefined){
					params['yuangongkaoqin'] = this.searchForm.yuangongkaoqin
				}
				if(this.searchForm.gongzuotaidu!='' && this.searchForm.gongzuotaidu!=undefined){
					params['gongzuotaidu'] = this.searchForm.gongzuotaidu
				}
				if(this.searchForm.yewujineng!='' && this.searchForm.yewujineng!=undefined){
					params['yewujineng'] = this.searchForm.yewujineng
				}
				if(this.searchForm.gongzuojixiao!='' && this.searchForm.gongzuojixiao!=undefined){
					params['gongzuojixiao'] = this.searchForm.gongzuojixiao
				}
				if(this.searchForm.jiangchengjiajianfen!='' && this.searchForm.jiangchengjiajianfen!=undefined){
					params['jiangchengjiajianfen'] = this.searchForm.jiangchengjiajianfen
				}
				if(this.searchForm.pingjiadengji!='' && this.searchForm.pingjiadengji!=undefined){
					params['pingjiadengji'] = this.searchForm.pingjiadengji
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "yuangongjixiao/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
...
<style lang="scss" scoped>
	...
	.el-table >>> .el-table__body-wrapper tbody tr td .view {
		border: 1px solid #0977fd60;
		cursor: pointer;
		border-radius: 0px;
		padding: 0 10px;
	...
	.el-table >>> .el-table__body-wrapper tbody tr td .view:hover {
		opacity: 0.8;
	}
	
	.el-table >>> .el-table__body-wrapper tbody tr td .add {
	}
	
	.el-table >>> .el-table__body-wrapper tbody tr td .add:hover {
	}
	
	.el-table >>> .el-table__body-wrapper tbody tr td .edit {
	...
	.el-table >>> .el-table__body-wrapper tbody tr td .edit:hover {
		opacity: 0.8;
	}
	
	.el-table >>> .el-table__body-wrapper tbody tr td .del {
		border: 1px solid #cc000060;
		cursor: pointer;
		border-radius: 0px;
		padding: 0 10px;
	...
	.el-table >>> .el-table__body-wrapper tbody tr td .del:hover {
		opacity: 0.8;
	}
	
	.el-table >>> .el-table__body-wrapper tbody tr td .btn8 {
		border: 1px solid #ed9a0d60;
		cursor: pointer;
		border-radius: 0px;
		padding: 0 10px;
	...
	.el-table >>> .el-table__body-wrapper tbody tr td .btn8:hover {
		opacity: 0.8;
	}
	...
	.main-content .el-table .el-switch >>> .el-switch__label--left {
		cursor: pointer;
		margin: 0 10px 0 0;
		color: #333;
		font-weight: 500;
	...
	.main-content .el-table .el-switch >>> .el-switch__label--right {
		cursor: pointer;
		margin: 0 0 0 10px;
		color: #333;
		font-weight: 500;
	...
	.main-content .el-table .el-switch >>> .el-switch__core {
	...
	.main-content .el-table .el-switch >>> .el-switch__core::after {
		border-radius: 100%;
		top: 1px;
		left: 1px;
		background: #fff;
	...
	.main-content .el-table .el-switch.is-checked >>> .el-switch__core::after {
		margin: 0 0 0 -18px;
		left: 100%;
	}
	
	.main-content .el-table .el-rate >>> .el-rate__item {
		cursor: pointer;
		display: inline-block;
		vertical-align: middle;
		font-size: 0;
	...
	.main-content .el-table .el-rate >>> .el-rate__item .el-rate__icon {
		margin: 0 3px;
		display: inline-block;
		font-size: 18px;
		position: relative;
