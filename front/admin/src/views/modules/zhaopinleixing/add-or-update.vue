<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="招聘类型" prop="zhaopinleixing" >
					<el-input v-model="ruleForm.zhaopinleixing" placeholder="招聘类型" clearable  :readonly="ro.zhaopinleixing"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="招聘类型" prop="zhaopinleixing" >
					<el-input v-model="ruleForm.zhaopinleixing" placeholder="招聘类型" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zhaopinleixing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zhaopinleixingCrossAddOrUpdateFlag = false;
											this.parent.search();
