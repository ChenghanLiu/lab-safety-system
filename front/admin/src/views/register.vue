	...
	<div>
		<div class="register-container">
			<el-form v-if="pageFlag=='register'" ref="ruleForm" class="rgs-form animate__animated animate__" :model="ruleForm" :rules="rules">
				<div class="rgs-form2">
					<div class="title">基于Python的安徽新华学院人事管理系统设计与实现</div>
					...
					<el-form-item class="list-item" v-if="tableName=='renshizhuguan'">
...
export default {
	...
	methods: {
		...
		login() {
			...
			if((!this.ruleForm.zhanghao) && `renshizhuguan` == this.tableName){
				this.$message.error(`账号不能为空`);
				return
			}
			if((!this.ruleForm.mima) && `renshizhuguan` == this.tableName){
				this.$message.error(`密码不能为空`);
				return
			}
			if((this.ruleForm.mima!=this.ruleForm.mima2) && `renshizhuguan` == this.tableName){
				this.$message.error(`两次密码输入不一致`);
				return
			}
			if((!this.ruleForm.xingming) && `renshizhuguan` == this.tableName){
			...
			if(`renshizhuguan` == this.tableName && this.ruleForm.lianxidianhua &&(!this.$validate.isMobile(this.ruleForm.lianxidianhua))){
				this.$message.error(`联系电话应输入手机格式`);
				return
			}
			this.$http({
